...
def process_large_file(input_file_path, output_file_path, lines_per_process=2000, num_processes=max(1, os.cpu_count() - 1)):
    print(f"{datetime.datetime.now().strftime("%H:%M:%S.%f")} 使用 {num_processes} 线程开始处理 {input_file_path}")

    with open(input_file_path, 'r', encoding='utf-8') as i:
        lines = i.readlines()

    chunks = [lines[i:i + lines_per_process] for i in range(0, len(lines), lines_per_process)]
